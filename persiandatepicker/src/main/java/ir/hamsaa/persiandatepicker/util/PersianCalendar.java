...
public class PersianCalendar extends GregorianCalendar {
	...
	public static YearMonthDay persianToGregorian(YearMonthDay persian) {
	...
	public String relativeFormattedPersianDate(){
		long diff = System.currentTimeMillis() - getTimeInMillis();
		long minutes = diff / 60_000L;
		long hours = minutes / 60;
		long days = hours / 24;

		if ( diff < 3600_000L ){ //less than an hour
			return Math.max(1,minutes) + " دقیقه پیش";
		} else if ( diff < 24 * 3600_000L ){ //less than a day
			return Math.max(1,hours) + " ساعت پیش";
		} else {



			return String.format(Locale.ENGLISH, "%d/%d/%d ساعت %02d:%02d", persianYear , persianMonth+1 , persianDay, get(HOUR_OF_DAY), get(MINUTE)) ;
		}
	}
}
